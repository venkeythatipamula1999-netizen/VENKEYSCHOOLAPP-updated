...
export default function DriverLeave({ onBack, currentUser }) {
  const staffId = currentUser?.role_id || '';
  const staffName = currentUser?.full_name || 'Driver';
  ...
  const balCards = [
    { label: 'Casual', key: 'casual', total: balance.casual || 12, color: C.teal },
    { label: 'Sick', key: 'sick', total: balance.sick || 12, color: C.gold },
      ...
      <View style={{ paddingHorizontal: 20, paddingBottom: 32 }}>
        {loading ? (
          ...
          <>
            ...
            <TouchableOpacity onPress={() => setShowModal(true)} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: C.teal, paddingVertical: 14, borderRadius: 14, marginBottom: 20 }}>
              <Icon name="leave" size={18} color={C.white} />
              <Text style={{ fontWeight: '600', fontSize: 15, color: C.white }}>Apply for Leave</Text>
            </TouchableOpacity>
              ...
              {submitted ? (
                ...
                <>
                    ...
                    <View style={{ backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 14, marginBottom: 10, overflow: 'hidden' }}>
                      {LEAVE_TYPES.map(t => (
                        <TouchableOpacity key={t} onPress={() => { setForm(p => ({ ...p, type: t })); setShowTypePicker(false); }} style={{ padding: 14, borderBottomWidth: 1, borderBottomColor: C.border }}>
                          <Text style={{ fontSize: 14, color: form.type === t ? C.teal : C.white }}>{t}</Text>
                        </TouchableOpacity>
                      ))}
                    </View>
                  )}
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                    ...
                    <TouchableOpacity onPress={handleSubmit} disabled={submitting} style={{ flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: C.teal, paddingVertical: 14, borderRadius: 14, opacity: submitting ? 0.7 : 1 }}>
                      {submitting ? <ActivityIndicator size="small" color={C.white} /> : <Icon name="check" size={16} color={C.white} />}
                      <Text style={{ fontWeight: '600', fontSize: 15, color: C.white }}>{submitting ? 'Submitting…' : 'Submit Request'}</Text>
                    </TouchableOpacity>
                  </View>
                </>
              )}
